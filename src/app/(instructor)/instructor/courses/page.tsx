...
async function loadInstructorCourses(): Promise<CourseRow[]> {
  ...
  if (error) {
    console.error('[courses] failed to load courses', { error, userId: user.id });
    ...
    return (
      <div className="mx-auto flex max-w-6xl flex-col gap-8 px-4 py-10 sm:px-6 lg:px-8">
        <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-3xl font-semibold">코스 관리</h1>
...
function CourseListItem({ course }: { course: CourseRow }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="text-xs text-muted-foreground">
        <p>생성: {createdAt.toLocaleDateString()}</p>
        <p>수정: {updatedAt.toLocaleDateString()}</p>
      </div>
    </div>
  );
}
